...
int wishExecute(char **args) {
    ...
    // pathNull is the reason that test 7 passed  
    if(!pathNull){
            return wishLaunch(args);  // If the command is not a built-in command, execute it
        }else{
            fprintf(stderr, "%s", error_message);
        }  
...
int validateArgs(char **args) {
    ...
    if (redirectCount > 1) {
        ...
        close(fout); // Close the file descriptor
        // Remove output file and redirect symbol from args
        args[i - 1] = NULL;
        args[i - 2] = NULL;
    }
    return 0;
